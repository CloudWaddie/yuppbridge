...
class TokenCache:
    ...
    def is_expired(self) -> bool:
        ...
        if self.last_updated is None:
            return True
        return datetime.now() - self.last_updated > timedelta(seconds=constants.TOKEN_CACHE_TTL)

    def is_valid(self) -> bool:
        """Check if cache has valid tokens"""
        return (
            not self.is_expired()
            and len(self.tokens) >= constants.MIN_REQUIRED_TOKENS
            and all(
...
class TokenExtractor:
    ...
    async def get_token(self, token_type: str) -> str:
        ...
        return constants.NEXT_ACTION_TOKENS.get(
            token_type, constants.NEXT_ACTION_TOKENS["new_conversation"]
        )
    ...
    async def mark_token_failed(self, token_type: str, token_value: str) -> None:
        ...
        async with self._lock:
            ...
            if self._cache.failed_attempts < constants.MAX_EXTRACTION_RETRIES:
    ...
    async def _attempt_extraction(self) -> bool:
        ...
        try:
            ...
            if extracted_tokens and len(extracted_tokens) >= constants.MIN_REQUIRED_TOKENS:
            ...
            print(f"[TokenExtractor] Extraction failed: {e}")
    ...
    async def _extract_from_chat_page(self) -> List[str]:
        ...
        try:
            ...
            if self.scraper:
                response = self.scraper.get(constants.YUPP_CHAT_URL, headers=headers, timeout=10)
                text = response.text
            else:
                ...
                if has_cloudscraper:
                    ...
                    if self.jwt_token:
                        scraper.cookies.set(constants.SESSION_TOKEN_COOKIE, self.jwt_token)
            ...
            if tokens:
                print(f"[TokenExtractor] Extracted {len(tokens)} tokens from chat page")
                return tokens

        except Exception as e:
            print(f"[TokenExtractor] Chat page extraction failed: {e}")
    ...
    async def _extract_from_main_page(self) -> List[str]:
        ...
        try:
            ...
            if self.scraper:
                response = self.scraper.get(constants.YUPP_BASE_URL, headers=headers, timeout=10)
                text = response.text
            else:
                if has_cloudscraper:
                    scraper = cloudscraper.create_scraper(
            ...
            tokens = self._extract_tokens_from_html(text)
            if tokens:
                print(f"[TokenExtractor] Extracted {len(tokens)} tokens from main page")
                return tokens

        except Exception as e:
            print(f"[TokenExtractor] Main page extraction failed: {e}")
                    ...
                    if any(pattern in script_url for pattern in bundle_patterns):
                        try:
                            ...
                            if tokens and len(tokens) >= constants.MIN_REQUIRED_TOKENS:
                                print(f"[TokenExtractor] Extracted tokens from JS bundle: {script_url}")
                                return tokens
                        except Exception:
                            continue

        except Exception as e:
            print(f"[TokenExtractor] JS bundle extraction failed: {e}")
    ...
    def _extract_tokens_from_html(self, html: str) -> List[str]:
        ...
        for pattern in constants.TOKEN_PATTERNS:
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        if self.jwt_token:
            headers["Cookie"] = f"{constants.SESSION_TOKEN_COOKIE}={self.jwt_token}"
...
def get_token_extractor(
    ...
    if _token_extractor_instance is None:
        _token_extractor_instance = TokenExtractor(jwt_token=jwt_token, scraper=scraper)
    return _token_extractor_instance


def reset_token_extractor() -> None:
    """Reset the token extractor (for testing)"""
    global _token_extractor_instance
    _token_extractor_instance = None
